...
func meal(food: String) -> Meal? {
    if let food = meals[food] {
        return food
    }
    else{
        return nil
 ...
 */
func userDefaultMeal() -> [String: Any]? {
    if let meals = UserDefaults.standard.dictionary(forKey: "meals") {
        print(meals)
        return meals
    }
    else {
