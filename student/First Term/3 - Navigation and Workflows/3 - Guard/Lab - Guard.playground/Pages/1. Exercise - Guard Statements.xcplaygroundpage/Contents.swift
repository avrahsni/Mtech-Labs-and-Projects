...
func createUser() -> User? {
    guard let fn = firstNameTextField.text, let ln = lastNameTextField.text, let age = ageTextField.text else {
        return nil
    }
    let user = User(firstName: fn, lastName: ln, age: age)
    return user
}
...
if let possibleUser = createUser() {
    print("The user named \(possibleUser.firstName) \(possibleUser.lastName) is \(possibleUser.age) years old")
}
/*:
